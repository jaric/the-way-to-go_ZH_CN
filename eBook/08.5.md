...
map 默認是無序的，不管是按照 key 還是按照 value 默認都不排序（詳見第 8.3 節）。

如果你想爲 map 排序，需要將 key（或者 value）拷貝到一個切片，再對切片排序（使用 sort 包，詳見第 7.6.6 節），然後可以使用切片的 for-range 方法打印出所有的 key 和 value。
